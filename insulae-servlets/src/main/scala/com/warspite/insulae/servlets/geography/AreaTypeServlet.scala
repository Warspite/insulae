...
object AreaTypeServlet {
  val PARAM_ID = "id";
}

class AreaTypeServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if (params.contains(AreaTypeServlet.PARAM_ID)) {
        return db.geography.getAreaTypeById(params.getInt(AreaTypeServlet.PARAM_ID)).asMap();
      }
      Map[String, Any]("areaTypes" -> db.geography.getAreaTypeAll());
    } catch {
