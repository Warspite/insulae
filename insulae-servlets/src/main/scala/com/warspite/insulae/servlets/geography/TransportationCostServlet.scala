...
class TransportationCostServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      Map[String,Any]("transportationCosts" -> db.geography.getTransportationCostAll());
    } catch {
