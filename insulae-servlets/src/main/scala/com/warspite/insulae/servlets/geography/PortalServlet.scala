...
object PortalServlet {
  val PARAM_AREA_ID = "areaId";
}

class PortalServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if (params.contains(PortalServlet.PARAM_AREA_ID)) {
        Map[String, Any]("locationNeighbors" -> db.geography.getLocationNeighborToOtherAreaByAreaId(params.getInt(PortalServlet.PARAM_AREA_ID)));
      } else {
        throw new MissingParameterException(false, PortalServlet.PARAM_AREA_ID);
      }
    } catch {
