  ...
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if(params.contains("id"))
        ...
        throw new ClientReadableException("Mandatory parameter 'id' or 'raceId' missing in request.", "Your request is missing parameters. Either 'id' or 'raceId' is required.");
      }
    } catch {
