...
class ItemTypeServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if (params.contains("id")) {
        db.industry.getItemTypeById(params.getInt("id")).asMap();
      } else {
        Map[String, Any]("itemTypes" -> db.industry.getItemTypeAll());
      }
    } catch {
