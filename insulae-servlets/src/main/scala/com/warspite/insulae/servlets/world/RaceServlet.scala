...
class RaceServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if(params.contains("id")) {
    	  db.world.getRaceById(params.getInt("id")).asMap();
      }
      else if (params.contains("realmId")) {
        Map[String,Any]("races" -> db.world.getRaceByRealmId(params.getInt("realmId")));
      }
      else {
        Map[String,Any]("races" -> db.world.getRaceAll());
      }
    } catch {
