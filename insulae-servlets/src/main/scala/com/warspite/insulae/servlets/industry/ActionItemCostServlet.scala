...
class ActionItemCostServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if (params.contains("actionId")) {
        Map[String, Any]("actionItemCosts" -> db.industry.getActionItemCostByActionId(params.getInt("actionId")));
      } else {
        Map[String, Any]("actionItemCosts" -> db.industry.getActionItemCostAll());
      }
    } catch {
