...
class TroubleReportTypeServlet(db: InsulaeDatabase, sessionKeeper: SessionKeeper) extends RequestHeaderAuthenticator(sessionKeeper) {
  override def get(request: HttpServletRequest, params: DataRecord): Map[String, Any] = {
    try {
      if (params.contains("id")) {
        db.meta.getTroubleReportTypeById(params.getInt("id")).asMap();
      } else {
        Map[String, Any]("troubleReportTypes" -> db.meta.getTroubleReportTypeAll());
      }
    } catch {
